...
export default class Anteater {
	...
	constructor(transform = null) {
		...
		const pelvis = this.pelvis = new Transform([ 0.0,  2.25, -1.5], [-2,   0,   0], [1, 1, 1], transform);
		...
		const lHand    = this.lFoot    = new Transform([0.0,  -1.1, -0.1], [-5.5, 0,   0], [1, 1, 1], lForearm);
		const rBicep   = this.rBicep   = lBicep.clone().translate(-1, 0, 0);
		const rForearm = this.rForearm = lForearm.clone().setParent(rBicep);
		const rHand    = this.rFoot    = lHand.clone().setParent(rForearm);
